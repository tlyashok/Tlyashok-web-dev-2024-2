...
def create_user():
    if request.method == 'POST':
        ...
        if errors:
            return render_template('create-user.html', roles=get_roles(), errors=errors)
        ...
        try:
            ...
            return render_template('create-user.html', roles=get_roles(), errors=errors)
    return render_template('create-user.html', roles=get_roles())
...
def edit_user(user_id):
    if request.method == 'POST':
        ...
        with db_connector.connect().cursor(dictionary=True) as cursor:
            cursor.execute(query, (user_id, ))
            user = cursor.fetchone()
    return render_template('edit-user.html', user=user, roles=get_roles())
